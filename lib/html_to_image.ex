defmodule HtmlToImage do
  ...
  def convert(html, options) do
    ...
    arguments = [
      "--format",
      Keyword.get(options, :format) || :jpg,
      "--width",
      Integer.to_string(Keyword.get(options, :width) || 1024),
      "--quality",
      Integer.to_string(Keyword.get(options, :quality) || 94),
      template_name,
      "-"
    ]
  ...
  defp executable_path do
    ...
    case result do
      ...
      _ ->
        Application.get_env(:html_to_image, :wkhtmltoimage_path) || "/usr/local/bin/wkhtmltoimage"
    end
  end
